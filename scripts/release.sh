...
remote=${GRAFHOME_CA_RELEASE_REMOTE:-origin}
branch=${GRAFHOME_CA_RELEASE_BRANCH:-main}
...
tag=$(
  unset GRAFHOME_CA_BUILD_VERSION GRAFHOME_CA_BUILD_TIMESTAMP GITHUB_REF_TYPE GITHUB_REF_NAME GITHUB_REF GITHUB_SHA
  GRAFHOME_CA_BUILD_COMMIT="$head_commit" scripts/release-tag.sh
)
tag_exists=0
...
# Use a lightweight tag on purpose. grafhome-ca release identity embeds the target
