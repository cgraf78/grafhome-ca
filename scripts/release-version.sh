...
# packaged metadata, and `grafhome-ca version`.
#
# grafhome-ca no longer has a hand-maintained VERSION file, but release binaries
...
if [[ -n "${GRAFHOME_CA_BUILD_VERSION:-}" ]]; then
  _valid_version "$GRAFHOME_CA_BUILD_VERSION" || _die "GRAFHOME_CA_BUILD_VERSION must look like YYYYMMDD-HHMMSS-<8hex>"
  version_commit=${GRAFHOME_CA_BUILD_VERSION##*-}
  if [[ "${commit:0:8}" != "$version_commit" ]]; then
    _die "GRAFHOME_CA_BUILD_VERSION commit suffix $version_commit does not match build commit ${commit:0:8}"
  fi
  printf '%s\n' "$GRAFHOME_CA_BUILD_VERSION"
  exit 0
fi
...
timestamp=${GRAFHOME_CA_BUILD_TIMESTAMP:-}
if [[ -z "$timestamp" ]]; then
...
_valid_timestamp "$timestamp" || _die "GRAFHOME_CA_BUILD_TIMESTAMP must look like YYYYMMDD-HHMMSS"
