...
: Root directory for CA-owned mutable state on the CA host. The untracked,
  owner-only enrollment key inventory is stored at
  `enrollments/registry.json`; include it in CA-origin backups. Approval and
  explicit enrollment import update the registry, while routine renewal does
...
- `revocations.toml`: optional versioned inventory of explicitly revoked SSH keys
...
Active key revocation uses a dedicated canonical document. A site that uses
`grafhome-ca revoke` must create and track the file before the first revoke;
keeping an initially empty file makes ownership explicit:
...
`kind = "host"` entries use `host` instead of `user` and `client_host`.
Public keys must use canonical `<algorithm> <base64>` form without comments.
The SSH fingerprint must match the public key, the renewal fingerprint is the
SHA-256 thumbprint of the retired public JWK, timestamps must be UTC RFC 3339
values ending in `Z`, and reasons are optional. The command writes this
document through a protected dot-managed symlink without replacing the symlink
itself. Treat both fingerprints as permanent tombstones: deleting them can
permit key reuse or re-enable a compromised key on hosts that have not retained
the older KRL.
...
user policy uses `schemas/policy/canonical/users.schema.json`, and revocations
use `schemas/policy/revocations.schema.json`; the original six-file schema URLs
remain compatibility entry points backed by the schemas in
...
`revoked_user_certs`
: Binary OpenSSH KRL generated by `grafhome-ca apply host` on SSH servers from
  user-key entries in `policy/revocations.toml`. `sshd` loads it through
  `RevokedKeys`.

`revoked_host_keys`
: Binary OpenSSH KRL generated by `grafhome-ca apply host` on SSH clients from
  host-key entries in `policy/revocations.toml`. `ssh` loads it through
  `RevokedHostKeys`. Valid installed KRLs are extended rather than rebuilt, so
  already-distributed revocations are not removed by an incomplete older policy
  snapshot.
